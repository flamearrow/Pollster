...
class TextTableViewController: UITableViewController, UITextViewDelegate
{
    ...
    override func tableView(_ tableView: UITableView, moveRowAt fromIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        if data != nil {
            data![destinationIndexPath.section].insert(data![fromIndexPath.section][fromIndexPath.row], at: destinationIndexPath.row)
            let fromRow = fromIndexPath.row + ((destinationIndexPath.row < fromIndexPath.row) ? 1 : 0)
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        if text.rangeOfCharacter(from: NSCharacterSet.newlines) != nil {
